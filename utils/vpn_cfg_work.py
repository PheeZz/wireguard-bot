...
class WireguardConfig:
    ...
    def generate_public_key(self, private_key: str, username: str, save: bool = True) -> str:
        ...
        try:
            ...
            if save:
                ...
                logger.info(f'[+] public key "{public_key}" for user {username} saved to database')
    ...
    async def get_last_peer_adress(self) -> str:
        ...
        try:
            ...
            logger.error(
                "[-] String 'AllowedIPs' not found in config file. Returning default adress"
                f" '10.0.0.2'. Error: {e.__repr__()}"
            )
            return "10.0.0.2"
    ...
    async def add_new_peer(self, username_and_device: str, peer_public_key: str) -> None:
        ...
        for line in (await self.get_config()).splitlines():
            if line.startswith("#") and line.endswith(username):
                for line2 in (await self.get_config()).splitlines():
                    if line2.startswith("AllowedIPs") or line2.startswith("#AllowedIPs"):
                        peer_address = line2
                # Remove 'AllowedIPs = ' from the string
                logger.info(f"[+] Peer address for user {username} is {peer_address[13:]}")
                return peer_address[13:] if line2.startswith("#AllowedIPs") else peer_address[14:]
        ...
        async with aiofiles.open(self.cfg_path, "r+") as cfg:
            ...
            for line_index, line in enumerate(config):
                if line.strip() in (f"#{username}_PC", f"#{username}_PHONE"):
                    ...
                    for line_under_username_index in range(line_index + 1, line_index + 5):
                        if line_under_username_index < len(config) and not config[
                            line_under_username_index
                        ].startswith("#"):
                            config[line_under_username_index] = (
